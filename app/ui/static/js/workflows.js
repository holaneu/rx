...
function renderMessageComponent({ title, body, data, form, isOpen = false, style }) { 
    const bodyHtml = body ? `<div class="message-body"><pre>${escapeHtml(body)}</pre></div>` : '';
    const dataHtml = data ? `<div class="message-data"><pre>${escapeHtml(data)}</pre></div>` : '';
    const formHtml = form ? `<div class="message-form">${form}</div>` : '';

    return `<div class="message">
        <details${isOpen ? ' open' : ''}>
            <summary style="${style}">${escapeHtml(title)}</summary>
            ${bodyHtml}
            ${dataHtml}
            ${formHtml}
        </details>
...
function handleMsg(response) {
    ...
    if (response.status === 'error') {
            ...
            domMessages.innerHTML += renderMessageComponent({
                isOpen: true,
                title: response.message.title,
                body: response.message.body,
                style: 'color: #f45b5b;'
